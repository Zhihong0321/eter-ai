 ...
 */
function parseLineItems(packageDescription) {
 ...
 */
function buildPriceView(context) {
    if (context.priceNet === null || context.priceNet <= 0)
        return null;
...
function renderPriceBlock(price) {
    ...
    if (price.discountAmount > 0) {
        tags.push(`<span class="ans-premium__price-tag ans-premium__price-tag--save">You save ${escapeHtml(formatRm(price.discountAmount))}</span>`);
    }
    if (price.discountPercent > 0) {
        tags.push(`<span class="ans-premium__price-tag">${escapeHtml(String(price.discountPercent))}% off</span>`);
    }
    for (const code of price.voucherCodes) {
        tags.push(`<span class="ans-premium__price-tag ans-premium__price-tag--voucher">${escapeHtml(code)}</span>`);
    }
    const wasLine = price.discountAmount > 0
 ...
 */
export function renderInvoicePackageCard(context) {
    const items = parseLineItems(context.packageDescription);
    const price = buildPriceView(context);
    const lead = 'Your complete rooftop solar package — premium equipment, all government applications, system design, installation, and electrical works, with full warranty and insurance protection included.';
    return [
...
//# sourceMappingURL=render-package-card.js.map