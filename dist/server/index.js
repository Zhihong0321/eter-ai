...
async function start() {
    ...
    if (totalTokens > 35000) {
        console.warn(`[server] WARNING: Token count (${totalTokens}) exceeds 35k budget. ` +
    ...
    app.listen(PORT, () => {
        ...
        if (!isProduction) {
            console.log('[server] Development mode — Vite handles the frontend (default http://localhost:5173)');
        }
    });
}
start().catch((err) => {
