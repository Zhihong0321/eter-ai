 ...
 */
function buildDistillSystemPrompt(category) {
 ...
 */
function parseDistillResponse(raw) {
...
async function getApiKeyFromVault() {
    ...
    const minimaxEntry = vault.credentials.find((c) => c.id === 'minimax' && c.type === 'api key');
...
/* ------------------------------------------------------------------ */
function execPromise(command, args, env, stdin, options = {}) {
 ...
 */
export async function distillContent(rawMarkdown, category) {
    if (!rawMarkdown.trim()) {
    ...
    const userPrompt = `Please distill the following content for the "${category}" category.\n\n` +
