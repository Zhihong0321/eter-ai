...
router.post('/api/feedback', async (req, res) => {
    ...
    const answerHtml = req.body?.answerHtml;
    const invoiceUid = req.body?.invoiceUid;
    ...
    const normalizedRating = rating === 'good' || rating === 'bad' ? rating : null;
    ...
    try {
        const saved = await saveFeedback({
            ...
            question,
            answerHtml,
            invoiceUid: typeof invoiceUid === 'string' && invoiceUid.trim()
                ? invoiceUid.trim()
                : null,
        ...
        console.error('[feedback] Could not save feedback:', err instanceof Error ? err.message : err);
