...
const CATEGORIES = ['products', 'schemes', 'company', 'basics'];
const router = Router();
/* ------------------------------------------------------------------ */
/*  Login (no auth required)                                           */
/* ------------------------------------------------------------------ */
router.post('/api/admin/login', (_req, res) => {
...
/* ------------------------------------------------------------------ */
router.get('/api/admin/knowledge', async (_req, res) => {
    try {
...
/* ------------------------------------------------------------------ */
router.get('/api/admin/manifest', async (_req, res) => {
    try {
        const raw = await readFile(MANIFEST_PATH, 'utf-8');
...
/* ------------------------------------------------------------------ */
router.get('/api/admin/tokens', async (_req, res) => {
    try {
        const raw = await readFile(MANIFEST_PATH, 'utf-8');
        const manifest = JSON.parse(raw);
...
/*  GET /api/admin/cache — FAQ cache entries                           */
/* ------------------------------------------------------------------ */
router.get('/api/admin/cache', async (_req, res) => {
    try {
...
/* ------------------------------------------------------------------ */
router.post('/api/admin/cache/regenerate', async (_req, res) => {
    try {
        const entries = await getCacheEntries();
...
/* ------------------------------------------------------------------ */
router.get('/api/admin/knowledge/:category/:file', async (req, res) => {
...
/* ------------------------------------------------------------------ */
router.put('/api/admin/knowledge/:category/:file', async (req, res) => {
    const { category, file } = req.params;
...
/*  POST /api/admin/knowledge/:category — create new file              */
/* ------------------------------------------------------------------ */
router.post('/api/admin/knowledge/:category', async (req, res) => {
    ...
    try {
        ...
        if (title || summary) {
            ...
            manifest.files.push({
                path,
                topic,
                title: title ?? topic.split('-').map((w) => w.charAt(0).toUpperCase() + w.slice(1)).join(' '),
                summary: summary ?? '',
                tokens,
...
/* ------------------------------------------------------------------ */
router.delete('/api/admin/knowledge/:category/:file', async (req, res) => {
    const { category, file } = req.params;
    if (!isValidCategory(category)) {
        res.status(400).json({ error: `Invalid category: ${category}` });
        return;
...
router.post('/api/admin/settings', async (req, res) => {
    ...
    if (LLM_API_KEY !== undefined)
        process.env.CHATBOT_API_KEY = LLM_API_KEY;
    if (LLM_MODEL !== undefined)
        process.env.CHATBOT_MODEL = LLM_MODEL;
    if (LLM_BASE_URL !== undefined)
        process.env.CHATBOT_BASE_URL = LLM_BASE_URL;
    try {
        ...
        if (LLM_API_KEY !== undefined)
            updateLine('CHATBOT_API_KEY', LLM_API_KEY);
        if (LLM_MODEL !== undefined)
            updateLine('CHATBOT_MODEL', LLM_MODEL);
        if (LLM_BASE_URL !== undefined)
            updateLine('CHATBOT_BASE_URL', LLM_BASE_URL);
