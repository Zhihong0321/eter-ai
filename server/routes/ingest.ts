...
const CATEGORIES = ['products', 'basics', 'company', 'schemes'] as const;
...
async function buildIngestResult(rawMarkdown: string, category: Category) {
  if (!rawMarkdown.trim()) {
...
router.post('/api/admin/ingest', async (req: Request, res: Response) => {
...
router.post('/api/admin/ingest/save', async (req: Request, res: Response) => {
  ...
  try {
    ...
    let manifest: { files: Array<{ path: string; topic: string; title: string; summary: string; tokens: number; loadOrder: number }> };
    try {
