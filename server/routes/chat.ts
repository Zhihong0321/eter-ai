...
import {
  buildInvoicePromptContext,
  buildSalesConsultantWhatsAppCta,
  getInvoicePackageContext,
  type InvoicePackageContext,
  InvoiceContextError,
...
function isElectricityBillSavingsQuestion(question: string): boolean {
  const normalised = normaliseQuestion(question);
  const hasBillContext = /\b(electric|electricity|tnb|bill|tariff|energy charge|monthly charge)\b/.test(
    normalised,
  );
  const hasSolarContext = /\b(solar|pv|panel|system|atap|nem)\b/.test(normalised);
  const asksForSavings =
    /\b(save|saves|saving|savings|saved|reduce|reduces|reduction|lower|lowers|cut|cuts|offset|offsets|eliminate|eliminates|roi|payback|return)\b/.test(
      normalised,
    );
  const asksForAmount =
    /\b(how much|amount|estimate|estimated|estimation|monthly|annual|per month|per year|rm|ringgit|%|percent|percentage)\b/.test(
      normalised,
    );
...
function configuredSalesConsultantWhatsAppUrl(): string {
  const raw =
    process.env.SALES_CONSULTANT_WHATSAPP_URL?.trim()
...
router.post('/api/chat', async (req: Request, res: Response) => {
  ...
  const rawInvoiceUid: unknown = req.body?.invoiceUid;
  ...
  const hasInvoiceUid =
    typeof rawInvoiceUid === 'string' && rawInvoiceUid.trim().length > 0;
  ...
  let invoiceContext: InvoicePackageContext | null = null;
  if (invoiceUid) {
    try {
  ...
  if (isElectricityBillSavingsQuestion(question)) {
    const whatsAppUrl =
      invoiceContext?.agentWhatsAppUrl || configuredSalesConsultantWhatsAppUrl();
    res.json({
      handoff: true,
