 ...
 */
function isDoneSignal(data: string): boolean {
  return data === '[DONE]';
}
...
export async function generateAnswer(
  ...
  if (!API_KEY) {
    return errorStream(
      'The AI service is not configured. Please set MINIMAX_API_KEY (or LLM_API_KEY) in the server environment and restart.',
    );
  }
  ...
  if (!response.ok) {
    ...
    return errorStream(
      `AI service returned HTTP ${response.status}. ${detail.slice(0, 300)}`,
    );
  }
